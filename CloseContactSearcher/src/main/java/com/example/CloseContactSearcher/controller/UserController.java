...
import com.example.CloseContactSearcher.jsr303.SetOperation;
...
public class UserController {
    ...
    @PreAuthorize(value = "hasAuthority('sys:manager')")
    @GetMapping("/showAllUsersByPages")
    public ResponseResult showAllUsersByPages(@RequestParam(defaultValue = "1") int pageNum, @RequestParam(defaultValue = "8") int pageSize){
        return new ResponseResult<>(200, "操作成功", userService.showAllUsersByPages(pageNum,pageSize));
    }
    ...
    public ResponseResult getInfo(@PathVariable String info){
            logger.info("进2");
